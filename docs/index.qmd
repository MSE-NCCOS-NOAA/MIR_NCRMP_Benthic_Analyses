...
TableCaption <- "<div style='font-size:1.15em; margin-bottom:0.5em; text-align: center;'>Table 1. Number of demographic (Demo) and benthic community assessment (BCA) surveys completed by M:IR, NCRMP, and DRM in 2022 and 2024.</div>"
...
if (knitr::is_html_output()) {
  ...
  cat(TableCaption)
  cat("![Summary Table](../Project/images/Proper_Table.png){ width=100% }")
}
```
...
sjPlot::tab_df(
  ...
  CSS = list(
    ...
    css.th = "font-style: normal; font-weight: bold;", 
...
The following figures for these metrics are grouped by each focal species. Significant statistical differences using a two-tailed t-test are indicated for differences between survey years (2022 vs. 2024) and between areas (MIR vs. NCRMP) for density, old mortality, and mean colony size.
...
a <- a %>%
  ...
  plt1 <- a %>%  
    ...
    labs(y = "Species", x = expression("Density (col/m"^2*")")) +
    theme_Publication(base_size = 10) +
    facet_grid(~YEAR) +
...
\*\* warning these estimates are not correct \*\*
...
```{r bleaching, fig.cap = "Figure 7. Species-specific bleaching frequency observed outside (NCRMP + DRM) and inside M:IR areas. Focal M:IR species are denoted with a (*).", fig.width=10, fig.asp=1.4, echo = FALSE}
...
plt_dis_MIR <- tmp_filtered %>%
 ...
 scale_pattern_manual(values = c("2022" = "none", "2024" = "stripe")) +
  geom_errorbar(aes(xmin = pmax(PV_LCI, 0), xmax = PV_UCI),
    ...
    labs(x = "Proportion of Colonies Diseased", y = "Coral Species", fill = "") +
    scale_x_continuous(expand = expansion(mult = c(0, 0.1))) +
        theme_Publication(base_size = 10) +
